...
select SERVICE_NAME,USERNAME  ,status,segtype,count(*) SESS_COUNT,sum( mb_used)  mb_used from
(   SELECT S.INST_ID,
         S.sid || ',' || S.serial# sid_serial,
         S.username,
         S.osuser,
...
GROUP BY s.inst_id,
         ...
         segtype
ORDER BY mb_used)
group by status,SERVICE_NAME,USERNAME,segtype;
...
-- 
-- 
---------Commented out after Bug 7210183: SQL_ID VALUE IS NOT SAME IN V$TEMPSEG_USAGE AND OTHER VIEWS. 
--prompt --prompt ##################################################################### 
