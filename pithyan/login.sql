...
--define default spool file
DEF _tpt_tempfile=sqlplus_tmpfile
col sqlplus_logfile new_value sqlplus_logfile noprint
col tpt_tempfile new_value _tpt_tempfile noprint

select
    to_char(sysdate, 'YYYYMMDD-HH24MISS') sqlplus_logfile
  , instance_name||'-'||to_char(sysdate, 'YYYYMMDD-HH24MISS') tpt_tempfile
from v$instance;

def sqlplus_logfile=C:\Users\admin\Documents\S\tmp\&_tpt_tempfile..log
-- spool sqlplus output
spool &sqlplus_logfile append
